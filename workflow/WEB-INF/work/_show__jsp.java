/*
 * JSP generated by Resin 2.1.16 (built Tue Feb 15 11:12:27 PST 2005)
 */

import javax.servlet.*;
import javax.servlet.jsp.*;
import javax.servlet.http.*;
import java.sql.*;
import javax.sql.*;
import java.util.*;
import com.msn.cbbp.workflow.*;

public class _show__jsp extends com.caucho.jsp.JavaPage{
  private boolean _caucho_isDead;
  
  public void
  _jspService(javax.servlet.http.HttpServletRequest request,
              javax.servlet.http.HttpServletResponse response)
    throws java.io.IOException, javax.servlet.ServletException
  {
    com.caucho.jsp.QPageContext pageContext = (com.caucho.jsp.QPageContext) com.caucho.jsp.QJspFactory.create().getPageContext(this, request, response, null, true, 8192, true);
    javax.servlet.jsp.JspWriter out = (javax.servlet.jsp.JspWriter) pageContext.getOut();
    javax.servlet.ServletConfig config = getServletConfig();
    javax.servlet.Servlet page = this;
    javax.servlet.http.HttpSession session = pageContext.getSession();
    javax.servlet.ServletContext application = pageContext.getServletContext();
    response.setContentType("text/html; charset=GBK");
    request.setCharacterEncoding("GBK");
    try {
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      pageContext.write(_jsp_string0, 0, _jsp_string0.length);
      

	WorkflowSession wfsession=SampleWorkflowSessions.getSampleWorkflowSession();
	Activity activity=wfsession.getCurrentActivity();
	
	List actionList =activity.getActionList();
	List inputList =activity.getInputList();
	


      pageContext.write(_jsp_string1, 0, _jsp_string1.length);
      out.print((activity.getLabel()));
      pageContext.write(_jsp_string2, 0, _jsp_string2.length);
      
	String actions="|";
	for(int i=0;i<actionList.size();i++){
		Action action=(Action)actionList.get(i);
		actions+=action.getEvent()+"|";
	
      pageContext.write(_jsp_string3, 0, _jsp_string3.length);
      out.print((action.getEvent()));
      pageContext.write(_jsp_string4, 0, _jsp_string4.length);
      out.print((action.getLabel()));
      pageContext.write(_jsp_string5, 0, _jsp_string5.length);
      out.print((action.getEvent()));
      pageContext.write(_jsp_string6, 0, _jsp_string6.length);
      out.print((action.getLabel()));
      pageContext.write(_jsp_string7, 0, _jsp_string7.length);
      
	
	}

      pageContext.write(_jsp_string8, 0, _jsp_string8.length);
      out.print((activity.getName()));
      pageContext.write(_jsp_string9, 0, _jsp_string9.length);
      out.print((activity.getLabel()));
      pageContext.write(_jsp_string10, 0, _jsp_string10.length);
      

	for(int i=0;i<inputList.size();i++){
		Input input=(Input)inputList.get(i);
		String value=wfsession.parseMessage(input.getValue());

      pageContext.write(_jsp_string11, 0, _jsp_string11.length);
      out.print((input.getType().equals("hidden")?"":input.getLabel()+":"));
      pageContext.write(_jsp_string12, 0, _jsp_string12.length);
      out.print((input.getType()));
      pageContext.write(_jsp_string13, 0, _jsp_string13.length);
      out.print((input.getName()));
      pageContext.write(_jsp_string14, 0, _jsp_string14.length);
      out.print((value));
      pageContext.write(_jsp_string15, 0, _jsp_string15.length);
      
	
	}

      pageContext.write(_jsp_string16, 0, _jsp_string16.length);
      out.print((actions));
      pageContext.write(_jsp_string17, 0, _jsp_string17.length);
      pageContext.include(activity.getPage()!=null?activity.getPage():"null");
      pageContext.write(_jsp_string18, 0, _jsp_string18.length);
      
	//WorkflowSession wfsession=SampleWorkflowSessions.getSampleWorkflowSession();
	inputList=wfsession.getVariantList();
	for(int i=0;i<inputList.size();i++){
		UserVariant input=(UserVariant)inputList.get(i);

      pageContext.write(_jsp_string11, 0, _jsp_string11.length);
      out.print((input.getName()));
      pageContext.write(_jsp_string19, 0, _jsp_string19.length);
      out.print((input.getValue()));
      pageContext.write(_jsp_string20, 0, _jsp_string20.length);
      
	
	}

      pageContext.write(_jsp_string21, 0, _jsp_string21.length);
      out.print((activity));
      pageContext.write(_jsp_string22, 0, _jsp_string22.length);
    } catch (java.lang.Throwable _jsp_e) {
      pageContext.handlePageException(_jsp_e);
    } finally {
      JspFactory.getDefaultFactory().releasePageContext(pageContext);
    }
  }

  private com.caucho.java.LineMap _caucho_line_map;
  private java.util.ArrayList _caucho_depends = new java.util.ArrayList();

  public boolean _caucho_isModified()
  {
    if (_caucho_isDead)
      return true;
    if (com.caucho.util.CauchoSystem.getVersionId() != -2089842219)
      return true;
    for (int i = _caucho_depends.size() - 1; i >= 0; i--) {
      com.caucho.vfs.Depend depend;
      depend = (com.caucho.vfs.Depend) _caucho_depends.get(i);
      if (depend.isModified())
        return true;
    }
    return false;
  }

  public long _caucho_lastModified()
  {
    return 0;
  }

  public com.caucho.java.LineMap _caucho_getLineMap()
  {
    return _caucho_line_map;
  }

  public void destroy()
  {
      _caucho_isDead = true;
      super.destroy();
  }

  public void init(com.caucho.java.LineMap lineMap,
                   com.caucho.vfs.Path appDir)
    throws javax.servlet.ServletException
  {
    com.caucho.vfs.Path resinHome = com.caucho.util.CauchoSystem.getResinHome();
    com.caucho.vfs.MergePath mergePath = new com.caucho.vfs.MergePath();
    mergePath.addMergePath(appDir);
    mergePath.addMergePath(resinHome);
    mergePath.addClassPath(getClass().getClassLoader());
    _caucho_line_map = new com.caucho.java.LineMap("_show__jsp.java", "/workflow/show.jsp");
    _caucho_line_map.add(6, 1);
    _caucho_line_map.add(1, 30);
    _caucho_line_map.add(51, 47);
    _caucho_line_map.add(60, 49);
    _caucho_line_map.add(66, 56);
    _caucho_line_map.add(66, 58);
    _caucho_line_map.add(68, 62);
    _caucho_line_map.add(71, 64);
    _caucho_line_map.add(78, 69);
    _caucho_line_map.add(78, 71);
    _caucho_line_map.add(86, 80);
    _caucho_line_map.add(87, 82);
    _caucho_line_map.add(87, 84);
    _caucho_line_map.add(87, 86);
    _caucho_line_map.add(88, 88);
    _caucho_line_map.add(92, 93);
    _caucho_line_map.add(98, 97);
    _caucho_line_map.add(104, 104);
    _caucho_line_map.add(104, 106);
    _caucho_line_map.add(113, 113);
    com.caucho.vfs.Depend depend;
    depend = new com.caucho.vfs.Depend(appDir.lookup("show.jsp"), 1156669422859L, 2413L);
    _caucho_depends.add(depend);
  }

  protected void _caucho_clearDepends()
  {
    _caucho_depends.clear();
  }

  private static byte []_jsp_string14;
  private static byte []_jsp_string8;
  private static byte []_jsp_string11;
  private static byte []_jsp_string20;
  private static byte []_jsp_string5;
  private static byte []_jsp_string1;
  private static byte []_jsp_string16;
  private static byte []_jsp_string6;
  private static byte []_jsp_string4;
  private static byte []_jsp_string0;
  private static byte []_jsp_string19;
  private static byte []_jsp_string21;
  private static byte []_jsp_string7;
  private static byte []_jsp_string9;
  private static byte []_jsp_string10;
  private static byte []_jsp_string2;
  private static byte []_jsp_string12;
  private static byte []_jsp_string13;
  private static byte []_jsp_string22;
  private static byte []_jsp_string18;
  private static byte []_jsp_string17;
  private static byte []_jsp_string15;
  private static byte []_jsp_string3;
  static {
    try {
      _jsp_string14 = "' value='".getBytes("GBK");
      _jsp_string8 = "\r\n\r\n<br>\r\n\r\n<font size='+3'>".getBytes("GBK");
      _jsp_string11 = "\r\n	".getBytes("GBK");
      _jsp_string20 = " <br>\r\n\r\n".getBytes("GBK");
      _jsp_string5 = "]</a>\r\n	<!--\r\n	<input type='submit' onclick=\"setEvent(this.form,'".getBytes("GBK");
      _jsp_string1 = "\r\n<script>\r\nfunction setEvent(form,val){\r\n	\r\n	form['event'].value=val;\r\n	form.submit();\r\n		\r\n	var elements=form.elements;	\r\n	for(var i=0;i<elements.length;i++){\r\n		\r\n		elements[i].disabled=true;\r\n		//alert(elements[i].type);\r\n	\r\n	}\r\n	\r\n	\r\n}\r\nfunction setEvent2(formName,val){\r\n	var form=document.forms[formName];\r\n	form['event'].value=val;\r\n	form.submit();\r\n		\r\n	var elements=form.elements;	\r\n	for(var i=0;i<elements.length;i++){\r\n		\r\n		elements[i].disabled=true;\r\n		//alert(elements[i].type);\r\n	\r\n	}\r\n	\r\n	\r\n}\r\n</script>\r\n<html>\r\n<head>\r\n<title>".getBytes("GBK");
      _jsp_string16 = "\r\n<input type='hidden' name='event' value='".getBytes("GBK");
      _jsp_string6 = "')\" value='".getBytes("GBK");
      _jsp_string4 = "')\">[".getBytes("GBK");
      _jsp_string0 = "\r\n".getBytes("GBK");
      _jsp_string19 = " = ".getBytes("GBK");
      _jsp_string21 = "<br>\r\n*/\r\n<hr>\r\n\r\n".getBytes("GBK");
      _jsp_string7 = "'>\r\n	\r\n	-->\r\n	".getBytes("GBK");
      _jsp_string9 = ": ".getBytes("GBK");
      _jsp_string10 = "</font>\r\n<br>\r\n".getBytes("GBK");
      _jsp_string2 = "\r\n</title>\r\n\r\n</head>\r\n\r\n<body>\r\n\r\n<form action='workflow.jsp' name='form2'>\r\n\r\n".getBytes("GBK");
      _jsp_string12 = " \r\n	<input type='".getBytes("GBK");
      _jsp_string13 = "' name='".getBytes("GBK");
      _jsp_string22 = "\r\n\r\n\r\n\r\n</body>\r\n</html>".getBytes("GBK");
      _jsp_string18 = "\r\n</form>\r\ncurrent variant:\r\n<hr>/*\r\n".getBytes("GBK");
      _jsp_string17 = "'>\r\n\r\n".getBytes("GBK");
      _jsp_string15 = "'>\r\n	".getBytes("GBK");
      _jsp_string3 = "\r\n	<a href=\"javascript:setEvent2('form2','".getBytes("GBK");
    } catch (java.io.UnsupportedEncodingException e) {
      e.printStackTrace();
    }
  }
}
